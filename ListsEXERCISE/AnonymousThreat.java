    ...
    public static void main(String[] args) {
        ...
        while (!command.equals("3:1")) {
            ...
            if (command.contains("merge")) {
                ...
                String textForDivision = texts.get(index);
                texts.remove(index);

                int countSymbolsPerPart = textForDivision.length() / parts;
                int beginIndex = 0;
                for (int part = 1; part < parts; part++) {
                    String textPerPart = textForDivision.substring(beginIndex, beginIndex + countSymbolsPerPart);
                    texts.add(index, textPerPart);
                    index++;
                    beginIndex += countSymbolsPerPart;

                }
                String textLastParts = textForDivision.substring(beginIndex, textForDivision.length());
